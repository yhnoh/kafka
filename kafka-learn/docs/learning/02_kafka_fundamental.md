...
Kafka는 이와 달리 ***토픽과 파티션이라는 계층적 구조를 통해 메시지를 디스크에 보관하면서도 높은 처리량을 유지*** 한다.
***메시지가 소비 후에도 삭제되지 않기 때문에 장애 복구 시 메시지 재처리가 가능하고, 파티션 단위로 컨슈머를 분배하여 순서를 보장하면서도 병렬 처리가 가능*** 하다. <br/>
